...
function inject (bot) {
  ...
  const windows = require('prismarine-windows')(bot.version).windows
  ...
  // Window(Chest/Furnace/etc...) opens
  bot._client.on('open_window', (packet) => {
    if(!['minecraft:chest', 'minecraft:dispenser'].includes(packet.inventoryType)) return;
    const window = bot.currentWindow = windows.createWindow(packet.windowId, packet.inventoryType, packet.windowTitle, packet.slotCount);
    if(!windowItems || window.id !== windowItems.windowId) {
      ...
      for(let i = 0; i < windowItems.items.length; ++i) {
        window.updateSlot(i, Item.fromNotch(windowItems.items[i]));
      }
      updateHeldItem();
      bot.emit('windowOpen', window);
    }
  ...
  bot._client.on('window_items', (packet) => {
    const window = packet.windowId === 0 ? bot.inventory : bot.currentWindow;
    if(!window || window.id !== packet.windowId) { windowItems = packet; return; }
    for(let i = 0; i < packet.items.length; ++i) {
      window.updateSlot(i, Item.fromNotch(packet.items[i]));
    }
    updateHeldItem();
  ...
  function putSelectedItemRange (start, end, window, slot, cb) {
    ...
    function tryToJoin () {
      ...
      const item = window.findItemRange(start, end, window.selectedItem.type, window.selectedItem.metadata, true);
      if (item) {
