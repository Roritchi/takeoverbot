...
function inject(bot) {
  ...
  const { objects, mobs, entitiesArray } = mc;
  ...
  bot.findPlayer = bot.findPlayers = (filter) => {
    ...
    const resultSet = Object.keys(bot.entities).map(key => bot.entities[key]).filter(filterFn);
  ...
  bot.entityIter = function() {
    return Object.keys(bot.entities).map(key => bot.entities[key]);
  }

  // Init our Player Entity
  ...
  bot._client.on('attach_entity', (packet) => {
    ...
    if (packet.vehicleId === -1) {
      bot.vehicle = null; // maybe use delete?
  ...
  bot._client.on('entity_metadata', (packet) => {
    ...
    entity.rawMetadata = packet.metadata;
  ...
  // Thunder Exploit bc everyone gets this packet
  ...
  /* #### Mostly Movement stuff but also interactions #### */
  ...
  bot._client.on('spawn_entity', (packet) => {
    ...
    entity.intField = packet.intField;
    entity.type = 'other';
    if (entityData) {
      entity.type = 'object';
    ...
    entity.uuid = packet.entityUUID || packet.objectUUID;
    entity.yaw = conv.fromNotchianYawByte(packet.yaw);
    entity.pitch = conv.fromNotchianPitchByte(packet.pitch);
  ...
  bot._client.on('player_info', (packet) => {
    ...
    packet.data.forEach((item) => {
      ...
      if (packet.action === 0) {
        ...
        playerEntity.displayName = item.displayName;
